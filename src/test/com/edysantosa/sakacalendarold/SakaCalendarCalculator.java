...
package com.edysantosa.sakacalendarold;
...
public class SakaCalendarCalculator {
	...
	public SakaCalendar hitungWuku(SakaCalendar tgl){
		...
		tgl.angkaWuku = (int)(pivot.angkaWuku + bedaHari)%210 ;
		if (tgl.angkaWuku < 0){
			tgl.angkaWuku = 210 - (int)((-(pivot.angkaWuku + bedaHari))%210) ;
		}

		if (tgl.angkaWuku <0){tgl.angkaWuku = -tgl.angkaWuku;}
	...
	public SakaCalendar hitungSaka(SakaCalendar tgl){
		...
		if (bedaHari >= 0){
			...
			if (hasilNgunaratri <= 15){
				...
				tgl.penanggal=hasilNgunaratri%15;
			}
			// Menghitung apakah penanggal atau pangelong
		...
		*/
		...
		if (tgl.get(Calendar.YEAR) > 2002 || tgl.get(Calendar.YEAR) < 1992 ){
			...
			if (bedaHari >= 0){
				...
				if(tgl.isNgunaratri && tgl.penanggal ==15 && !tgl.isNampih){tgl.noSasih--;}
				if(tgl.isNgunaratri && tgl.penanggal == 15 && tgl.noSasih == 9){tahunSaka--;} // penanggal 1 bertepatan dengan ngunaratri
